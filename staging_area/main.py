...
from skills import read_file, write_file, run_command
...
class EvoAgent:
    ...
    def evaluate_and_rewrite_core(self, params) -> str:
        if isinstance(params, dict):
            target_file = params.get("target_file", "")
            new_code = params.get("new_code", "")
            if not target_file or not new_code:
                return "Error: params dict missing 'target_file' or 'new_code'"
        else:
            try:
                ...
                return "Error: params must be a JSON string or dict with 'target_file' and 'new_code'"
    ...
    def _read_file(self, params) -> str:
        if isinstance(params, dict):
            path = params.get("path", str(params))
        else:
            path = str(params).strip().strip("'\"")
        return read_file(path)

    def _write_file(self, params) -> str:
        if isinstance(params, dict):
        ...
        try:
            ...
            return "Error: write_file requires JSON params with 'path' and 'content'"

    def _run_command(self, params) -> str:
        if isinstance(params, dict):
            cmd = params.get("command", str(params))
        else:
            cmd = str(params).strip().strip("'\"")
